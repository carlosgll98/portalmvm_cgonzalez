...
if ($_SESSION["rol"] == "alumne") {
    ...
    $select = "select * from cgonzalezmvm_portalmvm.vacants where practiques_dual='dual' or practiques_dual='FCT'";
    $vacants = mysqli_query($conn, $select);
    echo '
    <head>
		<title>Portal Alumne</title>
    ...
    while ($entrada = mysqli_fetch_array($vacants)) {
        ...
        echo '<td>
            <form action="" method="post">
                ...
                <button type="submit" name="Optar" value="'.$entrada['id'].'">Optar</button> 
            </form>
            </td>';
        echo "</tr>";
    }
...
elseif (isset($_POST['Optar'])){
    $_SESSION['linia']=$_POST['Optar'];
    $action = utf8_decode("INFO: El usuari " .$_SESSION["usermail"]. " ha optat a la vacant amb ID " .$_SESSION['linia'] );
    genlog($action);
